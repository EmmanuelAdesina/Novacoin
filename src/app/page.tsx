...
/* Selectable card for multi-choice questions */
function SelectCard({ label, sub, emoji, selected, onClick, multi = false, color = '#2D1F14' }: {
  label: string; sub?: string; emoji?: string; selected: boolean; onClick: () => void; multi?: boolean; color?: string;
}) {
  return (
    <motion.button
      onClick={onClick}
    ...
    >
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1 min-w-0">
          <span className={`font-semibold text-sm block ${selected ? 'text-[#2D1F14]' : 'text-[#2D1F14]/70'}`}>
            {label}
          </span>
  ...
  return (
    ...
    >
      ...
      <span className={`${isMd ? 'text-sm font-bold' : 'text-xs font-bold'} tracking-tight text-[#2D1F14]`}>
        Nova<span className="font-medium text-[#2D1F14]/50">coin</span>
      </span>
    </motion.div>
  );
}
  ...
  return (
    <div className="relative w-full max-w-xs mx-auto mt-10 md:mt-14 mb-4">
        ...
        <div className="flex flex-col items-center -my-1">
          {steps.slice(0, -1).map((_, i) => (
            <motion.div
              ...
              className="text-[#8B7D6B]/30 text-xs"
            >
              ↓
            </motion.div>
          ))}
      ...
      <motion.p
        ...
        className="text-center mt-4 text-[10px] text-[#8B7D6B]/50 italic"
      >
...
function InteractiveNetwork() {
  ...
  // Generate deterministic node positions based on count
    ...
    <div className="space-y-6">
        ...
        <svg viewBox="0 0 300 130" className="w-full h-auto" style={{ minHeight: 100 }}>
          ...
          {Array.from({ length: connectionCount }).map((_, i) => {
            ...
            return (
              <motion.line
                key={`c-${i}`}
                x1={nodes.current[a]?.x || 0}
                y1={nodes.current[a]?.y || 0}
                x2={nodes.current[b]?.x || 0}
                y2={nodes.current[b]?.y || 0}
                stroke={stage.color}
                strokeOpacity={0.12 + sliderVal * 0.02}
                strokeWidth="1"
      ...
      <div className="px-2">
        <input
          ...
          style={{
            background: `linear-gradient(to right, #C9503E ${(sliderVal / 5) * 100}%, #2D1F14/10 ${(sliderVal / 5) * 100}%)`,
          }}
        />
  ...
  return (
    <div ref={ref} className="max-w-sm mx-auto">
      <div className="space-y-4">
        {/* Context */}
        <FadeIn>
          <p className="text-center text-[15px] text-[#2D1F14]/70 font-medium">
            You owe your friend \u20A6100.
          </p>
        </FadeIn>
        ...
        <FadeIn delay={0.15}>
          <div className="bg-white rounded-3xl border border-[#2D1F14]/[0.06] shadow-[0_4px_24px_rgba(45,31,20,0.06)] overflow-hidden">
            ...
            <div className="px-6 py-5 space-y-5">
              ...
              >
                <div className="w-full bg-[#2D1F14] text-[#FFF9F5] font-semibold text-sm py-3 rounded-xl text-center">
                  SEND
                </div>
              </motion.div>
            </div>
        ...
        {/* Down arrow */}
        <FadeIn delay={0.5}>
          <div className="text-center text-[#8B7D6B]/30 text-lg">\u2193</div>
        </FadeIn>
...
function UseCases() {
  const cases = [
    ...
    { emoji: '\uD83D\uDED2', title: 'Pay', desc: 'Use it with businesses that accept it.', color: '#D4A574' },
...
/* ═══════════════════════════════════════════════════════════
   FEEDBACK: MULTI-STEP QUESTIONNAIRE
   ═══════════════════════════════════════════════════════════ */
...
export default function Page() {
  ...
  /* ─── Feedback state ─── */
  const [step, setStep] = useState(0); // 0-4 = questions, 5 = thanks, 6 = interest
  ...
  const submitFeedback = async () => {
    ...
    try {
      await fetch('/api/feedback', {
        ...
        body: JSON.stringify({
          type: 'feedback',
          q1_what_is_it: q1WhatIsIt,
          q2_use_cases: q2UseCases,
          q3_why_not: q3WhyNot,
          q4_would_try: q4WouldTry,
          open_feedback: feedbackText,
        }),
      });
      ...
      >
        <div className="max-w-5xl mx-auto px-5 py-3.5 flex items-center justify-between">
          ...
          <button
            ...
            className="md:hidden ml-3 p-2 cursor-pointer"
            aria-label="Menu"
          >
        ...
        <AnimatePresence>
          {mobileMenuOpen && (
            <motion.div
              ...
              className="md:hidden overflow-hidden border-t border-[#2D1F14]/[0.04] bg-[#FFF9F5]/95 backdrop-blur-lg"
            >
            ...
            <div className="bg-white rounded-3xl p-8 md:p-12 border border-[#2D1F14]/[0.04] shadow-[0_2px_20px_rgba(45,31,20,0.04)] text-center relative overflow-hidden">
              ...
              <div className="relative">
                ...
                <div className="mt-8 space-y-4">
                  <p className="text-[15px] text-[#2D1F14]/70 leading-relaxed">
                    That&apos;s exactly what we&apos;re trying to understand.
                  </p>
                  <p className="text-[15px] text-[#2D1F14] font-medium leading-relaxed">
                    What would make a new digital asset useful enough that people would actually choose it?
                  </p>
                  <p className="text-sm text-[#8B7D6B]/60 italic">
                    We don&apos;t know the answer yet. That&apos;s why we need your honest feedback.
                  </p>
                </div>
              </div>
            </div>
          ...
          <FadeIn>
            <div className="flex items-start gap-3 bg-[#C9503E]/[0.04] rounded-2xl p-5 border border-[#C9503E]/10">
              ...
              <div>
                ...
                <p className="text-[13px] text-[#2D1F14]/60 leading-relaxed">
                  This isn&apos;t being presented as a guaranteed investment or a way to make money. We&apos;re exploring whether a digital asset can become useful because lots of people use it.
                </p>
              </div>
            </div>
          </FadeIn>
      ...
      <section className="py-16 md:py-20 px-5">
        <div className="max-w-3xl mx-auto text-center">
          <FadeIn>
            ...
            <h2 className="text-2xl md:text-3xl font-black tracking-tight">
              Maybe it starts with one university.
            </h2>
            <p className="mt-4 text-[#8B7D6B] max-w-md mx-auto text-[15px] leading-relaxed">
              A few students. Then a few communities. Then thousands of people. We want to see what happens.
            </p>
          </FadeIn>
          ...
          <FadeIn delay={0.3}>
            <p className="mt-8 text-[15px] text-[#2D1F14]/70 max-w-sm mx-auto leading-relaxed">
              Could something like this grow from a small community into something much bigger?
            </p>
          </FadeIn>
        </div>
      </section>
          ...
          <AnimatePresence mode="wait">
            ...
            {step === 0 && (
              ...
              >
                <div className="text-center mb-8">
                  ...
                  <p className="mt-3 text-[#8B7D6B] text-sm max-w-sm mx-auto">
                    4 quick questions. No sign-up required. Takes about 30 seconds.
                  </p>
                </div>
                <button
                  onClick={() => setStep(1)}
                  className="w-full bg-[#2D1F14] text-[#FFF9F5] font-semibold text-sm py-4 rounded-2xl hover:bg-[#3D2F24] transition-all cursor-pointer"
                >
                  Let&apos;s go
                </button>
              </motion.div>
                ...
                <div className="space-y-3">
                  {[
                    ...
                    <SelectCard
                      key={opt.value}
                      label={opt.label}
                      selected={q1WhatIsIt === opt.value}
                      onClick={() => { setQ1WhatIsIt(opt.value); setTimeout(() => setStep(2), 300); }}
                    />
                  ))}
                </div>
              </motion.div>
            ...
            {step === 2 && (
              ...
              >
                ...
                <div className="space-y-3">
                  {[
                    ...
                    <SelectCard
                      key={opt.value}
                      label={opt.label}
                      selected={q2UseCases.includes(opt.value)}
                      multi
                ...
                <button
                  onClick={() => setStep(3)}
                  className="mt-6 w-full bg-[#2D1F14] text-[#FFF9F5] font-semibold text-sm py-3.5 rounded-xl hover:bg-[#3D2F24] transition-all cursor-pointer"
                >
                  Continue
                </button>
              </motion.div>
            ...
            {/* ── STEP 3: BIGGEST REASON YOU MIGHT NOT USE IT ── */}
            {step === 3 && (
              <motion.div
                key="s3"
                initial={{ opacity: 0, x: 40 }}
                ...
                <div className="space-y-3">
                  {[
                    ...
                    <SelectCard
                      key={opt.value}
                      label={opt.label}
                      selected={q3WhyNot === opt.value}
                      onClick={() => { setQ3WhyNot(opt.value); setTimeout(() => setStep(4), 300); }}
                    />
                  ))}
                </div>
              </motion.div>
                ...
                <div className="space-y-3">
                  {[
                    ...
                    <SelectCard
                      ...
                      emoji={opt.emoji}
                      label={opt.label}
                      selected={q4WouldTry === opt.value}
                      onClick={() => { setQ4WouldTry(opt.value); setTimeout(() => setStep(5), 300); }}
                    />
                  ))}
                </div>
              </motion.div>
                ...
                {!submitted ? (
                  <>
                    ...
                    <div className="bg-white rounded-2xl p-6 border border-[#2D1F14]/[0.04] shadow-[0_1px_12px_rgba(45,31,20,0.04)]">
                      ...
                      <button
                        ...
                        disabled={submitting}
                        className="mt-4 w-full bg-[#2D1F14] text-[#FFF9F5] font-semibold text-sm py-3.5 rounded-xl hover:bg-[#3D2F24] disabled:opacity-40 disabled:cursor-not-allowed transition-all cursor-pointer"
                      >
                  ...
                  <>
                    ...
                    <div className="text-center">
                      ...
                      <p className="mt-4 text-xs text-[#8B7D6B]/40">
                        You&apos;re one of the people helping us test the idea.
                      </p>
                    </div>
                    ...
                    <div className="mt-8 bg-white rounded-2xl p-6 border border-[#2D1F14]/[0.04] shadow-[0_1px_8px_rgba(45,31,20,0.03)]">
                      ...
                      <div className="grid grid-cols-4 gap-2">
                        <a
                          href={`https://wa.me/?text=${encodeURIComponent('Check out this idea I found - NovaCoin: ' + shareUrl)}`}
                          target="_blank"
                        ...
                        <a
                          href={`https://twitter.com/intent/tweet?text=${encodeURIComponent('Check out this idea - NovaCoin')}&url=${encodeURIComponent(shareUrl)}`}
                          target="_blank"
                          rel="noopener noreferrer"
                        ...
                        <a
                          href={`https://t.me/share/url?url=${encodeURIComponent(shareUrl)}&text=${encodeURIComponent('Check out this idea - NovaCoin')}`}
                          target="_blank"
                          rel="noopener noreferrer"
            ...
            {step === 6 && (
              ...
              >
                ...
                <p className="mt-3 text-[#8B7D6B] text-sm">
                  If this idea becomes something real, you&apos;ll be among the first to know.
                </p>
              </motion.div>
            )}
          </AnimatePresence>
